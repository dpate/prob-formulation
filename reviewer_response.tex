...
"Reconfigurability in MDO Problem Synthesis". Full name added to the paper. 
...
This paragraph was poorly worded. We have re-worded it to attempt 
to make the issue with REMS more clear and to address more bluntly 
what it fails to allow. 
...
equation numbers were added, see new equations 8 and 9. Also, verbal descriptions
were added to accompany the set notations. 
...
We chose not to address the issue of seleting one FPG over another. The answer 
to this quetion depends heavily on specific situation. If you're short on time, 
a smaller FPG with less cycles and less variables may be preferable. If you're trying
to resolve some small issue with nacelle-wing interference then you may choose to go with a
more high fidelity and computationally expensive FPG. 

Our main point was that the MCG provides a good foundation for deriving the FPG, and that 
it can be revisited later in the design process as things change. You are not stuck modifying the 
FPG you initally select. 
...
We agree that this sort of quantitative data would be very valuable. However, we 
contend that this data needs to be larger in scope than just that which could be obtained 
by executing the example problem given here. Instead we think that to do that topic 
justice, you would need to address a wide range of problems with different graph structural 
characteristics. The challenge is that any quantitative measures given are inherantly 
dependent on particular details of a problem. So to address this question fully you 
would need a range of problems to consider. This work, while very valuable, would 
fill an entire paper in it's own right and it's something we will consider in future 
work. 
...
Althought the length of the paper was not excessive, we agree that the conclusion 
could be more concise. We have shortened the conclusion and re-woreded it to make 
it more clear. 
