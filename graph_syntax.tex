...
\item[expression:] simple algebraic combinations of variables (such as for 
  constraint or objective functions)
...
is the number of fixed or explicit edges directed out and is denoted as $\txt{deg}^+(v)$.
We also define the \emph{upper in-degree limit} as
\begin{equation}
...
\end{equation}
These user-specified limits govern the number of explicit edges that may be directed into a variable
...
\begin{description}
  ...
  \item[collision: ] The number of incomming edges is greater than the upper indegree limit:
...
edge joining them to a model node. These variables are local because they are explicitely tied
...
would be considered a design variable in the graph. When working with an MCG or FPG, only fixed and explicit edges are allowed, 
in which case a design variable is simply a variable node with no incomming edges. For a valid PSG 
any design variable node would have at least one incomming 
implicit edge from a driver node. 

((DP: I think we are struggling to define the exact boundary between a hole and design variable and when one can become the other.
This stems from the fact that they are identical with respect to the graph structure: they are both variable nodes representing local inputs without any edges directed in.
Therefore, I suggest we let the difference be strictly determined by the user--set indegree limits: given a node in question, it is a hole if the lower indegree limit is greater than zero. 
Then, a few things can happen. A) the hole can become irrelevant if the analysis block is not needed. B) the hole can cause the graph to be invalid and the user can change the lower indegree limit or change the maximal connectivity graph to include a new edge (and presumably other new nodes and edges as well). 
C) the user could have set the \emph{upper} indegree limit to be zero, meaning that it is supposed to be a design variable.
Having written this, I feel that (if you agree) we should probably have it in the paper somewhere)).
...
Fig.~\ref{f:designvars} may have an objective function of $z^2$ or $2y + z$. Constraints are 
always given in the form of either an inequality or an equality, e.g., 
...
((DP:I think we can probably just say ``alegbraic expression'' and not have to show any, especially since the variables aren't given meaning))
...
((DP: it is not clear to me how the difference between local and global should change in the process of obtaining the FPG from the MCG. For example,
in the MCG a node representing a global input may have several edges directed out to multiple analysis blocks, but in the FPG all but one have been removed. 
From my point of view, I was thinking that the difference between global and local wasn't a node's current state, but what it has the potential to be. Another way of saying this is: the definition of global vs local shouldn't depend on explicit edges because they are free to be removed, unless we also want a node to be able to change from being global or local. Lastly, a global input in an FPG that only has one edge directed out could be removed and the node it was directed in to could become a design variable.))

((DP: along these lines, what if a design variable is a separate variable node outside of the analysis block. Then it doesn't need any indegree limits and being global or local can be directly tied to the number of edges directed out?))
