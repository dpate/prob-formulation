...
In addition, we introduce the 
  ...
  as opposed to the flexible relationship between variables from different analyses. 
\end{comment}
...
%  Following our definition any variable node that had multiple 
...
automatically result in the correct CO structure.
  ...
  Analysis and expression blocks are also regarded local or global but the 
...
%  only outgoing connection edges are relevant. 
...
%A path, given by $P=(V_\txt{path},E_\txt{path})$, contains a cycle if at least two edges in the path refrence the same node,  $E = \{(x_0,x_1),(x_1,x_2),\ldots,(x_{k-1},x_k),(x_k,x_0)\}$. 
%Cycles decribe the reciprical dependence of a set of two or more analyses on each other's outputs. 
