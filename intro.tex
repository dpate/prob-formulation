...
\subsection{Specific vs Fundamental Problem Formulation }
    ...
    Here, $D_1$ and $D_2$ represent analysis tools, $F$, $G_1$, and $G_2$ are the objective 
    and constraint functions respectively, and $H$ is the coupling constraint. 
    Equation \ref{eqn:simple} implies the execution 
    order for the problem. Analysis $D_1$ outputs $y_1$, which is an input to $D_2$. 
    Hence, $D_1$ should be run before $D_2$ with with $y_2$ iterating
    to convergence with $\hat{y_2}$. A slightly different formulation 
    is equally valid and still represents the same problem: 

    \begin{align}
        \txt{given} & \ \ y_2 = D_2(\hat{y_1},z_1,z_2) \notag
        \\      & \ \ y_1 = D_1(x_1,y_2,z_1,z_2) \notag
        \\\txt{min.} &\ \ F(x_1,y_1,y_2,z_2) \notag
        \\\txt{w.r.t.} & \ \ x_1,\hat{y_1},y_2,z_1,z_2 \notag
        \\\txt{s.t.} & \ \ H(y_1,\hat{y_1}) = 0 \notag 
        \\     & \ \ G_1(y_1) \geq 0 \notag
        \\     & \ \ G_2(y_2) \geq 0
        \label{eqn:simple2}
    \end{align}
    ...
    $\hat{y_2}$, while Eq. \ref{eqn:simple2} includes $\hat{y_1}$. The result is that
...
Equations \ref{eqn:simple} and 
    \ref{eqn:simple2}  are specific alterative expressions of a more fundamental problem description 
    ...
    \begin{align}
        ...
        \\\txt{min.} &\ \ F(x_1,y_1,y_2,z_2) \notag
        \\\txt{w.r.t.} & \ \ x_1,\hat{y_1},\hat{y_2},z_1,z_2 \notag
        \\\txt{s.t.} & \ \ H_1(y_1,\hat{y_1}) = 0 \notag
        \\     & \ \ H_2(y_2,\hat{y_2}) = 0 \notag 
        \\     & \ \ G_1(y_1) \geq 0 \notag
        \\     & \ \ G_2(y_2) \geq 0
    ...
    The formulation of Eq. \ref{eqn:simple_fpf} differs from both Eq. \ref{eqn:simple} 
    and Eq. \ref{eqn:simple2} because it includes two coupling constraints. 
The result is that Eq. \ref{eqn:simple_fpf} requires the presence of both 
    $\hat{y_1}$ and $\hat{y_2}$, which means that $D_1$ and $D_2$ each now operate with 
    their own local copies of the coupling variables. This formulation fully decouples the problem so that 
...
Figure \ref{f:sellar graph} depicts the Sellar problem given by Eq.~\ref{eqn:simple_fpf} in the graph-based syntax which will be presented in this paper. This example will serve to highlight the features of the syntax as they are introduced.
\begin{figure}[htb!]
  \begin{center}
    \includegraphics[width=4.0in]{images/sellar}
  \end{center}
  \caption{Sellar problem represented as a graph.}
\label{f:sellar graph}
\end{figure} 
...
For example, the objective, $f$, from the FPF in Eq. \ref{eqn:simple_fpf} is dependent on 
    output $m$ of analysis $A$ ((BG: I don't quite get this)). The dependence of $m$ on $A$ is now captured 
