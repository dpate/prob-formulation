...
\section{Using DSM for Problem Formulation}
    ...
    [TODO: There is more recent research to cite here, like Allison's Work.]

    Lamb and Martins included the variables, objectives, and constraint functions as nodes in an Extended 
    DSM (XDSM) in order to capture a more complete problem formulation for MDAO problems\cite{Lambe2012}. By adding in those 
    ...
    Although XDSM captures part of the FDT formulation, it still leaves out the variables themselves. As a result, all variable information is aggregated 
    ...
    constraints necessary to implement the problem. Hence, to complete the DSM for use with a fundamental problem formulation you need 
    to add in nodes for each variable in the analysis. In the following sections we describe the necessary syntax for specifying a 
    problem formulation with DSM syntax and show how to make use of the resulting graph to find the FPF from any given graph. 
