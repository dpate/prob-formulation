...
\section{Existing Graph--Based Syntax}
    ...
    Thought it does not contain specific variable information on the diaganol of the DSM, this could be easily added back in
    simply by putting the variables back into the graph and drawing the appropriate connections
    ...
    stripping it of the useful information contained in an FDT. We propose
    a new graph syntax that retains the features of XDSM, without the dependence on 
    solution strategy (although it does allow for that information to be present). This 
    new syntax allows for the compelete specification of an MDAO problem.